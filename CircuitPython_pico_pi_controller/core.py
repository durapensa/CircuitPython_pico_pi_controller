...
__all__ = ['UNDevice', 'PPDevice', 'PPController', 'IDENTITY', 'BUFCLR', 'CLR', 'IDF', 'BOS', 'TIM', 'CMD', 'HOS',
           'LOD', 'TZO', 'PEN', 'MSG', 'NAM', 'ICS', 'UID', 'RPT', 'PPD']
...
class PPDevice():
    ...
    def get_cmd(self):
        ...
        with self.i2cdevice as i2cdevice:
            ...
            msg = bytearray(1)
            try:
                i2cdevice.write_then_readinto(CMD,msg)
...
class PPController():
    ...
    def __init__(self, **kwargs):
        ...
        cmd_codes = (
            ...
            (  1, 'OFFLINE', 1),        # confirm
            (  2, 'ONLINE', 1),         # confirm
            (  3, 'DEREGISTER', 1),     # confirm
            (  4, 'HOSTNAME', 1),       # confirm
            ( 63, 'FLICKER', 1),        # duration
            ( 64, 'ROUNDROBIN', 1),     # duration
            ( 65, 'REPORT', 1),         # number of PPDs
            (127, 'MIBOSMANG', 1),      # confirm

            (252, 'REBOOT', 1),         # confirm
            (253, 'SHUTDOWN', 1),       # confirm
            (254, 'POWERON', 1),        # confirm
            (255, 'POWEROFF', 1)        # confirm
        )
        """Command code int values, command NAME, number of bytes to expect
           from follow-on request"""
    ...
    def png_ppds(self,ppds=None):
        """Ask PPDs for their essential stats."""
        fname='png_ppds'
